...
??? question "You may be wondering right now ''Wait, didn't Blender use **diffs**?''"
    Didn't Blender use **key map item diffs** to merge user modifications from
    the **user key config** into the **active key config**?".
...
What we do in the case where we need to refine the comparison is:
...
??? example "You may inspect the list of fingerprints from the add-on's preferences panel"
    Navigate to `Preferences > Add-ons > Keyboard Layout Emulation > Debug add-on preferences`.
    
    ![Debug add-on preferences > Remapped keymaps](images/debug-addon-preferences-remapped-keymaps.png)
    
    Alternatively, the `wm.kle_export_addon_preferences` operator (the `Export...` button above)
    has some options (marked as only for debug), to include this dictionary in the exported JSON file.
...
???+ tip "Blender supports more fine-grained control over modifier matching than the UI suggests"
    ...
    appear to enable all modifiers.
...
!!! info "The JSON list of fingerprints takes around 100 kiB of space for a factory Blender installation"
...
!!! info
    ...
    this task is always scheduled with a `0` seconds delay, to try to postpone it until
    Blender is done loading the add-on.
    
    To improve the chances that we catch any add-on shortcuts that were registered a little late,
    we also reapply emulation a few seconds afterwards, to catch any add-ons that were
    registered after us.
...
!!! tip "Performance"
    Reapplying emulation for just a few shortcuts often has an imperceptible performance impact.
    This add-on should not perceptibly increase the time Blender takes to start.
...
!!! tip "Performance"
    ...
    (even though checking for changes in enabled add-ons is a relatively lightweight
    ...
    The polling interval of 1 second can be adjusted, but there should be no need.
...
!!! quote "After all, Blender is open source. Why not fix the key map system if it is so impractical?"
